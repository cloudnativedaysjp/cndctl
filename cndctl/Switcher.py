...
class Switcher:
    def __init__(self, nextcloud_base_path: str, uploader_base_path: str) -> None:
        """Switcher関連の初期化
    ...
    def __create_scenecollection(self, scenecollection_name: str) -> None:
        ...
        self.requests.append(simpleobsws.Request('CreateSceneCollection', {
            'sceneCollectionName': scenecollection_name
        }))

    # シーンとシーンに紐づくソースをまとめて作成する
    def __create_presentation_scene(self, scene_name: str, input_kind: str, input_settings: dict) -> None:
        """シーンとシーンに紐づくソースをまとめて作成する
        ...
        """
        # シーンの作成  
        self.requests.append(simpleobsws.Request('CreateScene', {
            'sceneName': scene_name
        }))

        # インプットソースを作成
        self.requests.append(simpleobsws.Request('CreateInput', {
            'sceneName': scene_name,
        ...
        self.requests.append(simpleobsws.Request('CreateSourceFilter', {
    ...
    def __create_maintenance_scene(self) -> None:
        ...
        self.requests.append(simpleobsws.Request('CreateInput', {
            ...
            'inputSettings': {
                'local_file': self.nextcloud_base_path + '/Sync/Media/z-common/please_wait(調整中).mp4',
                'looping': True
            }
        }))
            ...
            "playlist": [
                {
                    ...
                    "hidden": False,
                    "selected": False,
                    "value": f"{self.nextcloud_base_path}/Sync/Media/broadcast-{talk['track_name']}/makuai/0.mp4"
                },
                {
                    # CM をディレクトリとして追加
                    "hidden": False,
                    "selected": False,
                    "value": "{self.nextcloud_base_path}//Sync/Media/z-common/cm"
                },
            ]
        }
        ...
        for day in talks_days:
            ...
            for track in day['tracks']:
                ...
                for request in self.requests:
                    ...
                    print(ret)
                    if ret.ok():
                        print(f"Request succeeded! Request:{request} Response:{ret}")
                    else:
                        logger.error(f"Request error. Request:{request} Response:{ret}")
                        sys.exit()
