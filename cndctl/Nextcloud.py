...
class Nextcloud:
    ...
    def dirsync(self):
        ...
        # print(talks)
        talks = csv.DictReader(
            open(self.EVENT_TALK_FILE_PATH, encoding="utf_8", mode="r")
        )
        ...
        with NextCloud(
            ...
            for talk in talks:
                ...
                path = nxc.get_folder(
                    path=f"{self.NEXTCLOUD_DIR_PATH}{dir_name.replace('/', '_')}"
                )["href"][27:-1]

                share_data = list(
                    filter(lambda item: item["path"] == path, nxc.get_shares().data)
                )
                share = ""
                ...
                if len(share_data) == 0:
                    ...
                    if not self.DRY_RUN:
                        ...
                        print(share)
                        nxc.update_share(
                            sid=share["id"], public_upload=True, permissions=4
                        )
                        print(
                            talk["id"]
                            + ","
                ...
                if not self.DRY_RUN:
                    if not self.dk.set_video_registration(
                        talk["id"], share_data[0]["url"]
                    ):
